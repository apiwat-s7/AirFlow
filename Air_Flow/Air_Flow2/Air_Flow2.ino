...
void loop(void) {
    ...
    if(S1 == HIGH)
    {
        ...
        dac_value = adcValueRead*3.8;//True sum 4.0 increase 0.1 | *1.5
        
        dac_expected_output = (5.0/4096.0) * dac_value;// 0.00 - 5.00 "convert to volt"
        //dac.setVoltage(dac_value, false);// 0 - 1024     

        if(dac_expected_output<0.001)
        {
          dac_expected_output = 0.00;
          dac.setVoltage(40.95, false);
        }
        else
        {
          dac.setVoltage(dac_value, false);//0-4095
        }
    ...
    {   
        adcValueRead = analogRead(voltsIn);// 0 - 1024
        voltageRead = (adcValueRead * 5.0 )/ 1024.0;// 0.00 - 5.00 "convert to volt"
        dac_value = adcValueRead*3.5;//True sum 4.0 increase 0.1
    ...
    else if(S3 == HIGH)
    {
        adcValueRead = analogRead(voltsIn);// 0 - 1024
        voltageRead = (adcValueRead * 5.0 )/ 1024.0;// 0.00 - 5.00 "convert to volt"
        dac_value = adcValueRead*2.0;//True sum 4.0 increase 0.1
